...
export class UserService {
  ...
  async createUser(createUserDetails: createUserDto): Promise<{ status: boolean, message: string }> {
    createUserDetails.password = await UserService.hashPassword(createUserDetails.password);
    return await this.userModel.create({ ...createUserDetails }).then(() => {
